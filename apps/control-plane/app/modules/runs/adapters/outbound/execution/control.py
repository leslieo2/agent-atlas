...
from app.modules.runs.adapters.outbound.execution.launchers import K8sLauncher
from app.modules.runs.adapters.outbound.execution.specs import runner_run_spec_from_run_spec
...
class ExecutionControlRegistry(ExecutionControlPort):
    ...
    def cancel_run(self, request: CancelRequest) -> bool:
        run = self._get_run_backend(request.run_id)
        if run is None:
            return False
        return run.cancel_run(request)

    def retry_run(self, run_id: str | UUID) -> RunHandle | None:
        backend = self._get_run_backend(run_id)
        if backend is None:
            return None
        return backend.retry_run(run_id)
    ...
    def _get_run_backend(self, run_id: str | UUID) -> _ExecutionBackendAdapter | None:
        for backend in self.backends.values():
            snapshot = backend.get_status(run_id)
