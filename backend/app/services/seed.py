...
def bootstrap() -> None:
    # Ensure idempotent bootstrap during process reload.
    with state.lock:
        if state.datasets:
            return
    ...
    dataset = Dataset(
        ...
        rows=[
            ...
            DatasetSample(sample_id="sample-002", input="What is fastest carrier from PVG to SFO?"),
        ],
    )
    support = Dataset(
    ...
    with state.lock:
        for run in seeded_runs:
            ...
            state.save_run(run)
        for run_id, steps in seeded_steps.items():
            state.trajectory[run_id] = steps
            state.persist_trajectory(run_id)
        state.datasets[dataset.name] = dataset
        state.save_dataset(dataset)
        state.datasets[support.name] = support
        state.save_dataset(support)
