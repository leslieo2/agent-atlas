...
def test_run_execution_projector_builds_success_trace_event():
    ...
    assert record.event.span_id == f"span-{run_id}-1"
    assert record.event.parent_span_id is None
...
def test_run_execution_projector_handles_prompt_only_run():
    ...
    assert record.event.input["prompt"] == "Explain the plan."
    assert record.event.input["model"] == "gpt-4.1-mini"
